...
import { PackagePlus, DollarSign, AlignLeft, Send, UploadCloud, ImageIcon, Loader2, Type, Quote, Star } from "lucide-react";
...
export default function AddProductPage() {
  ...
  // States for the new review images preview
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // 2. Helper function to compress arrays of images
      ...
      // ضغط باقي الصور (المعرض والتقييمات)
  ...
  return (
    <div className="relative flex-col min-h-screen w-full flex items-center py-20 justify-center overflow-hidden bg-[#0f172a]">
      ...
      <motion.div 
        ...
        className="relative z-10 w-full max-w-2xl p-6 mx-4" // Widened max-w to fit more fields
      >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5" dir="rtl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative group">
                 <PackagePlus className="absolute left-4 top-3.5 w-5 h-5 text-blue-300" />
                 <input name="name" type="text" placeholder="اسم الباقة الرئيسية" className="w-full bg-slate-900/50 border border-white/10 p-3 pl-12 rounded-xl focus:outline-none focus:border-cyan-400" required disabled={isSubmitting} />
              </div>
              <div className="relative group">
                 <DollarSign className="absolute left-4 top-3.5 w-5 h-5 text-blue-300" />
                 <input name="price" type="number" placeholder="السعر" className="w-full bg-slate-900/50 border border-white/10 p-3 pl-12 rounded-xl focus:outline-none focus:border-cyan-400" required disabled={isSubmitting} />
              </div>
            </div>

            <div className="relative group">
               <AlignLeft className="absolute left-4 top-3.5 w-5 h-5 text-blue-300" />
               <textarea name="description" rows={2} placeholder="وصف المنتج (الأساسي)" className="w-full bg-slate-900/50 border border-white/10 p-3 pl-12 rounded-xl focus:outline-none focus:border-cyan-400" disabled={isSubmitting} />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative group">
                 ...
                 <input name="hookTitle" type="text" placeholder="العنوان العريض (مثال: مصاحف برواية ورش)" className="w-full bg-slate-900/50 border border-white/10 p-3 pl-12 rounded-xl focus:outline-none focus:border-cyan-400" disabled={isSubmitting} />
              </div>
              <div className="relative group">
                 <Type className="absolute left-4 top-3.5 w-5 h-5 text-cyan-300" />
                 <input name="hookSubtitle" type="text" placeholder="العنوان الفرعي (مثال: مع توصيل سريع)" className="w-full bg-slate-900/50 border border-white/10 p-3 pl-12 rounded-xl focus:outline-none focus:border-cyan-400" disabled={isSubmitting} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Main Image */}
              <div className="relative group border-2 border-dashed border-slate-500 hover:border-cyan-400 rounded-xl transition bg-slate-800/50 p-4">
                <label className="block text-center text-xs text-cyan-400 font-bold mb-2">الصورة الرئيسية *</label>
              ...
              {/* Gallery */}
              <div className="relative group border-2 border-dashed border-slate-500 hover:border-cyan-400 rounded-xl transition bg-slate-800/50 p-4">
                 <label className="block text-center text-xs text-cyan-400 font-bold mb-2">صور المعرض</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="relative group border-2 border-dashed border-slate-500 hover:border-yellow-400 rounded-xl transition bg-slate-800/50 p-4">
                 <label className="block text-center text-xs text-yellow-400 font-bold mb-2 flex justify-center gap-1"><Star size={14}/> صور تقييمات (القسم 1)</label>
              ...
              <div className="relative group border-2 border-dashed border-slate-500 hover:border-yellow-400 rounded-xl transition bg-slate-800/50 p-4">
                 <label className="block text-center text-xs text-yellow-400 font-bold mb-2 flex justify-center gap-1"><Star size={14}/> صور تقييمات (القسم 2)</label>
            ...
            <motion.button 
              ...
              className={`mt-4 w-full text-white font-bold py-4 rounded-xl shadow-lg flex items-center justify-center gap-2 ${isSubmitting ? 'bg-slate-600 cursor-not-allowed' : 'bg-gradient-to-r from-blue-600 to-cyan-500'}`}
            >
              {isSubmitting ? (
                <><span>جاري الضغط والرفع...</span><Loader2 className="w-5 h-5 animate-spin" /></>
              ) : (
                <><span>حفظ المنتج المكتمل</span><Send className="w-5 h-5" /></>
              )}
            </motion.button>
