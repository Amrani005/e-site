...
import { createProducts } from "@/actions/createProducts"; // تأكد من المسار الصحيح
...
export default function AddProductPage() {
  ...
  // حالة التحميل لنخبر المستخدم أننا نضغط الصور
  ...
  // 🧠 العقل المدبر: هذه الدالة تعترض الإرسال وتضغط الصور أولاً
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault(); // نمنع تحديث الصفحة الافتراضي
    setIsSubmitting(true); // تشغيل أيقونة التحميل
    ...
    try {
      ...
      // --- إعدادات الضغط ---
      const options = {
        maxSizeMB: 0.04, // أقصى حجم للصورة 300 كيلوبايت (خفيفة جداً!)
        maxWidthOrHeight: 1080, // تصغير الأبعاد لتناسب الشاشات
        useWebWorker: true,
      };
      ...
      if (imageFile && imageFile.size > 0) {
        console.log("حجم الصورة الرئيسية قبل الضغط:", imageFile.size / 1024 / 1024, "MB");
        const compressedImage = await imageCompression(imageFile, options);
        console.log("حجم الصورة بعد الضغط:", compressedImage.size / 1024 / 1024, "MB");
        // استبدال الصورة القديمة بالجديدة المضغوطة
        formData.set("image", compressedImage, compressedImage.name);
      }

      // 2. ضغط صور المعرض
      const galleryFiles = formData.getAll("gallery") as File[];
      formData.delete("gallery"); // نحذف الصور الثقيلة القديمة من الصندوق
      
      for (const file of galleryFiles) {
        if (file.size > 0) {
          const compressedFile = await imageCompression(file, options);
          // نضيف الصور المضغوطة واحدة تلو الأخرى
